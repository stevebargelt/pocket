...
## layer & thumb-cluster drills — hand-curated (Pocket) [v1.2]

v1.2 adds two keymap-aware practice modes — **layer drills** (practice text
restricted to the characters a chosen ZMK layer binds) and **thumb-cluster
drills** (targeted sequences over the Glove 80's thumb keys). Both reuse the
existing per-context corpora plus the weakness engine for their bulk text; on top
of that they ship a small **hand-curated seed** so symbol/number layers and
thumb-key sequences still have material when no corpus line happens to fit.

- **Source:** `Pocket (hand-authored)`
- **License:** `MIT`
- **Where it lives:** unlike the per-context corpora, these seeds are **not**
  `data/seeds/*.json` rows — they are short inline constants `LAYER_SEEDS`
  (`src/server/drills/layerDrills.ts`) and `THUMB_SEEDS`
  (`src/server/drills/thumbDrills.ts`).
- **Provenance:** Original symbol/punctuation sequences and thumb-key bigrams
  (space/enter/backspace + whatever symbols the keymap binds on the thumbs),
  written for Pocket and chosen for typing practice. Nothing is sampled or
  excerpted from an external dataset, so **no third-party attribution applies**.
  Pure-modifier and `&mo`/`&lt`/`&to`/`&tog` layer-toggle thumb keys are not
  typeable, so they are surfaced **visually in the live keymap (guided,
  unscored)** rather than seeded as drill text.
- **No runtime fetch / no raw dump.** Consistent with every other corpus here,
  nothing is downloaded at runtime; these seeds are authored in-repo.

### Keymap source files are never committed

The parsed `.keymap` is **the user's own layout**, read from a git-ignored
`keymaps/` directory and **never redistributed** (full note in `../CREDITS.md`).
Pocket resolves the file from `POCKET_KEYMAP_PATH` or the most-recently-modified
`*.keymap` under `keymaps/`; no exported MoErgo layout is ever committed to this
repo, and the ZMK keymap format it parses is MIT (ZMK firmware — see
`../CREDITS.md`).
